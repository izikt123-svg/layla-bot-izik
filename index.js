...
const express = require('exconst { Client, LocalAuth } = require('whatsapp-web.js');
const qrcode = require('qrcode-terminal');
const { GoogleGenerativeAI } = require("@google/generative-ai");
...
client.on('message', async message => {
    ...
    try {
        ...
        const result = await model.generateContent(`You are Layla, Itzik's strategic partner at GoTours. Answer briefly in Hebrew: ${message.body}`);
        await message.reply(result.response.text());
    } catch (e) { console.error(e); }
});

client.initialize();press');
